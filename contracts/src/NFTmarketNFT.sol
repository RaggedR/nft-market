...
contract NFTmarketNFT is ERC721, ERC721URIStorage, ERC721Royalty, Ownable, ReentrancyGuard {
    ...
    event LicenseAgreementSigned(
        uint256 indexed tokenId,
        address indexed signer,
        bytes32 newLicenseHash
    );
    ...
    /**
     * @notice Buy a listed token (requires license agreement signature)
     * @param tokenId Token to buy
     * @param licenseSignature Signature of the license agreement message
     */
    function buy(uint256 tokenId, bytes memory licenseSignature) external payable nonReentrant {
        Listing storage listing = listings[tokenId];
        ...
        // Verify the buyer signed the license agreement
        bytes32 messageHash = getLicenseMessageHash(tokenId, msg.sender);
        address signer = ECDSA.recover(messageHash, licenseSignature);
        if (signer != msg.sender) {
            revert InvalidLicenseSignature();
        }
        ...
        // Update license hash with new buyer's signature
        bytes32 newLicenseHash = keccak256(abi.encodePacked(messageHash, licenseSignature));
        tokenData[tokenId].licenseHash = newLicenseHash;
        ...
        emit LicenseAgreementSigned(tokenId, msg.sender, newLicenseHash);
    ...
    /**
     * @notice Get the license message that must be signed to purchase a token
     * @param tokenId The token being purchased
     * @param buyer The address of the buyer
     * @return The message that must be signed
     */
    function getLicenseMessage(uint256 tokenId, address buyer) public view returns (bytes memory) {
        TokenData memory data = tokenData[tokenId];
        string memory licenseTypeName;

        if (data.licenseType == LicenseType.Display) {
            licenseTypeName = "Display";
        } else if (data.licenseType == LicenseType.Commercial) {
            licenseTypeName = "Commercial";
        } else {
            licenseTypeName = "Transfer";
        }

        return abi.encodePacked(
            "I agree to the NFTmarket license agreement.\n",
            "Token ID: ", _toString(tokenId), "\n",
            "License Type: ", licenseTypeName, "\n",
            "Buyer: ", _toHexString(buyer), "\n",
            "Image Hash: ", _toHexString(data.imageHash)
        );
    }

    /**
     * @notice Get the hash of the license message for signing
     * @param tokenId The token being purchased
     * @param buyer The address of the buyer
     * @return The hash to sign (with Ethereum signed message prefix)
     */
    function getLicenseMessageHash(uint256 tokenId, address buyer) public view returns (bytes32) {
        bytes memory message = getLicenseMessage(tokenId, buyer);
    ...
    /**
     * @dev Converts a bytes32 to its hex string representation
     */
    ...
    /**
     * @dev Converts a uint256 to its hex string representation with fixed length
     */
